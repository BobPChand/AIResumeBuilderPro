...
// On iOS, subscriptions to unlock in-app digital features MUST go through
// Apple's native In-App Purchase (StoreKit), not an external payment link.
// This screen routes iOS purchases through RevenueCat (react-native-purchases).
// Before shipping to the App Store: create the Monthly/Yearly products in
// App Store Connect, configure matching entitlements/offerings in the
// RevenueCat dashboard, and add the RevenueCat public iOS SDK key below.
// Web and Android may continue to use Stripe Checkout directly.

const REVENUECAT_IOS_KEY = ''; // TODO: add RevenueCat public iOS SDK key before App Store submission
...
export default function PaywallScreen() {
  ...
  const handleSubscribeWeb = async () => {
    setLoading(true);
    try {
      const data = await startCheckout({ plan: selected });
      if (data.url) {
  ...
  const handleSubscribeIOS = async () => {
    if (!REVENUECAT_IOS_KEY) {
      Alert.alert(
        'Almost there',
        'In-app purchases are being finalized for iOS. Please check back shortly, or subscribe from our website in the meantime.'
      );
      return;
    }
    // When REVENUECAT_IOS_KEY is set and react-native-purchases is configured,
    // replace this with a real Purchases.purchasePackage() call using the
    // offering/package that matches `selected` (monthly/yearly).
  };
        ...
        <TouchableOpacity style={styles.ctaButton} onPress={handleSubscribe} disabled={loading}>
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.ctaSub}>Then {selected === 'monthly' ? 'CA$14.99/month' : 'CA$119.99/year'} · Cancel anytime</Text>
            </>
          )}
        </TouchableOpacity>

        <Text style={styles.footer}>No charge for 7 days · Cancel anytime</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
